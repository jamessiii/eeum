...
export function getWorkspaceGuide(state: AppState, workspaceId: string): WorkspaceGuide {
  ...
  const steps: GuideStep[] = [
    ...
    createStep({
      ...
      allowedInteractionSelectors: [
        '[data-guide-target="transactions-uncategorized-category-input"] input',
    ...
    createStep({
      ...
      title: "설정 화면 소개",
      description:
        "**설정 화면은 백업과 테마 같은 운영 기능을 모아둔 곳입니다.** 자주 편집하지 않아도 위치는 알아두는 편이 좋습니다.",
      targetPath: "/settings",
    ...
    createStep({
      ...
      title: "설정 상단 요약",
      description:
        "**설정 상단 카드에서는 목표 저축률과 지출 경고 기준을 바로 확인하고 고칠 수 있습니다.** 숫자가 지금 생활 리듬에 맞는지 먼저 보고, 필요하면 연필 버튼으로 바로 수정해보면 됩니다.",
      targetPath: "/settings",
    ...
    createStep({
      id: "settings-theme-return",
      title: "라이트 모드로 다시 돌리기",
      description:
        "**테마 버튼을 한 번 더 눌러 원래 보던 라이트 모드로 돌아와보세요.** 실제 사용 중에는 바꿔본 뒤 다시 익숙한 화면으로 복귀하는 흐름도 자주 쓰게 됩니다.",
      targetPath: "/settings",
      targetSelector: '[data-guide-target="settings-theme-toggle"]',
      fallbackSelector: '[data-guide-target="settings-theme"]',
      ctaLabel: "라이트 모드 보기",
      activeLabel: "다시 눌러보세요",
      tips: ["이제 마지막 완료 안내를 보고 튜토리얼을 마칩니다."],
      completed: false,
      requiresTargetVisit: true,
      blocking: true,
      interactionKind: "press",
      interactionLabel: "라이트 모드로 다시 바꿔보세요",
      allowedInteractionSelectors: ['[data-guide-target="settings-theme-toggle"]'],
    }),
  ];
